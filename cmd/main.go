...
// Package main is the entry point for the OpenStack baremetal operator.
package main
...
import (
	...
	"github.com/openstack-k8s-operators/openstack-baremetal-operator/internal/controller"
...
func main() {
	...
	if err := (&controller.OpenStackProvisionServerReconciler{
	...
	if err := (&controller.OpenStackBaremetalSetReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(mgr); err != nil {
	...
	if strings.ToLower(os.Getenv("ENABLE_WEBHOOKS")) != "false" {
		...
		if err = webhookv1beta1.SetupOpenStackBaremetalSetWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "OpenStackBaremetalSet")
			os.Exit(1)
		}

		if err = webhookv1beta1.SetupOpenStackProvisionServerWebhookWithManager(mgr); err != nil {
