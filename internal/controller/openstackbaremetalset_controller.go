...
// +kubebuilder:rbac:groups=core,resources=secrets,verbs=create;delete;get;patch;update
// +kubebuilder:rbac:groups=core,resources=secrets/finalizers,verbs=update;patch
...
func (r *OpenStackBaremetalSetReconciler) reconcileNormal(ctx context.Context, instance *baremetalv1.OpenStackBaremetalSet, helper *helper.Helper) (ctrl.Result, error) {
	...
	if instance.Spec.OSImageDeploymentType == baremetalv1.OSImageDeploymentTypeSelfExtracting {
		// SelfExtracting mode: create or use provision server
		...
		// PassThrough mode: validate OSContainerImageURL and skip provision server
		if instance.Spec.OSContainerImageURL == "" {
			instance.Status.Conditions.Set(condition.FalseCondition(
				condition.InputReadyCondition,
				condition.ErrorReason,
				condition.SeverityWarning,
				"OSContainerImageURL is required for PassThrough mode"))
			return ctrl.Result{RequeueAfter: time.Second * 10}, nil
		}
		// PassThrough mode: skip provision server, mark condition as ready
		instance.Status.Conditions.MarkTrue(baremetalv1.OpenStackBaremetalSetProvServerReadyCondition, "Provision server not needed for PassThrough mode")
	}
